...
//    m_bconnectionLost = false;
    ...
    if(clientPersistentData->owner.getUin() && !getUseMD5()) {
        ...
        log(L_DEBUG, "Login %lu [%s]", clientPersistentData->owner.getUin(), /*pswd.c_str()*/"");
        QString uin;
        uin.sprintf("%lu", clientPersistentData->owner.getUin());
    ...
    if (!clientPersistentData->owner.getScreen().isEmpty() || getUseMD5()){
        log(L_DEBUG, "Requesting MD5 salt");
        ByteArrayBuilder builder;
        builder.appendDword(1);
        oscarSocket()->flap(OscarSocket::FlapChannelNewConnection, builder.getArray());

        TlvList list;

        if(clientPersistentData->owner.getUin()) {
            QString uin = QString::number(clientPersistentData->owner.getUin());
            list.append(Tlv(0x0001, uin.toUtf8()));
        } else {
            list.append(Tlv(0x0001, clientPersistentData->owner.getScreen().toUtf8()));
        }
        list.append(Tlv(0x004B, QByteArray()));
        list.append(Tlv(0x005A, QByteArray()));
        oscarSocket()->snac(ICQ_SNACxFOOD_LOGIN, ICQ_SNACxLOGIN_AUTHxREQUEST, 0, list.toByteArray());
        return;
    }
//    if (m_bVerifying){
...
//    }
    ...
    // I guess the following is deprecated
//    flap(ICQ_CHNxNEW);
//    socket()->writeBuffer() << 0x00000001L;
//    sendPacket(true);
...
void ICQClient::chn_close(const QByteArray& data)
{
