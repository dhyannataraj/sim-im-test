...
std::string JabberClient::getConfig()
{
    QString lr;
    ...
    setListRequest(lr);
    std::string res = Client::getConfig();
    if (res.length())
        res += "\n";
...
bool JabberClient::send(Message *msg, void *_data)
{
    ...
    case MessageAuthRefused:{
            ...
            if (data->Subscribe.toULong() & SUBSCRIBE_FROM){
                ...
                << (const char*)encodeXML(msg->getPlainText()).utf8()
                << "</status></presence>";
                sendPacket();
    ...
    case MessageTypingStart:
        if (getTyping()){
            ...
            //<< msg_id.c_str()
            << (const char*)data->TypingId.str().utf8()
            << "</id></x></message>";
            sendPacket();
            delete msg;
            return true;
    ...
    case MessageTypingStop:
        if (getTyping()){
            ...
            << (const char*)data->TypingId.str().utf8()    //msg_id.c_str()
            << "</id></x></message>";
            sendPacket();
            delete msg;
            return true;
