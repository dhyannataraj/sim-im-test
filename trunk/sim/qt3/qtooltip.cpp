...
void QTipManager::removeFromGroup( MyQToolTipGroup *g )
{
    QPtrDictIterator<QTipManager::Tip> i( *tips );
    QTipManager::Tip *t;
    while( (t = i.current()) != 0 ) {
...
void MyQToolTip::initialize() // ## remove 3.0
{
}

void MyQToolTip::cleanup() // ## remove 3.0
{
}
...
void MyQToolTip::setFont( const QFont &font )
{
...
void MyQToolTip::setPalette( const QPalette &palette )
{
...
MyQToolTip::MyQToolTip( QWidget * parent, MyQToolTipGroup * group )
{
    p = parent;
    g = group;
    initTipManager();
...
void MyQToolTip::remove( QWidget * widget )
{
    if ( tipManager )
...
void MyQToolTip::add( QWidget * widget, const QRect & rect, const QString &text )
{
    initTipManager();
...
void MyQToolTip::remove( QWidget * widget, const QRect & rect )
{
    if ( tipManager )
...
void MyQToolTip::tip( const QRect & rect, const QString &text )
{
    initTipManager();
    tipManager->add( parentWidget(), rect, text, 0, QString::null, 0, TRUE );
}

void MyQToolTip::tip( const QRect &geometry, const QRect &rect, const QString &text )
{
    initTipManager();
...
MyQToolTipGroup::MyQToolTipGroup( QObject *parent, const char *name )
        : QObject( parent, name )
{
    del = TRUE;
    ena = TRUE;
...
MyQToolTipGroup::~MyQToolTipGroup()
{
    if ( tipManager )
...
bool MyQToolTipGroup::delay() const
{
    return del;
}
...
void MyQToolTipGroup::setDelay( bool enable )
{
#if 0
    if ( enable && !del ) {
...
void MyQToolTipGroup::setEnabled( bool enable )
{
    ena = enable;
}
...
/*!
  ...
  \sa setEnabled()
*/
bool MyQToolTipGroup::enabled() const
{
    return (bool)ena;
}
...
void MyQToolTip::setEnabled( bool enable )
{
