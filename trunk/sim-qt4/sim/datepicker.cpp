        ...
        : QFrame(this, Qt::Popup | Qt::Tool )
{
    ...
    pal.setColor(QPalette::Text, QColor(127, 0, 0));
    pal.setColor(QPalette::Foreground, QColor(255, 0, 0));
    QFont f(font());
    f.setBold(true);

    m_labels = new QLabel*[7 * 6];
    QGridLayout *lay = new QGridLayout(lbl);
    lay->setMargin(6);
    lay->setSpacing(4);
    unsigned n = 0;
...
{
    unsigned year  = atol(m_yearBox->text().toLatin1());
    unsigned month = m_monthBox->value() + 1;
    unsigned day   = atol(lbl->text().toLatin1());
...
void PickerPopup::monthChanged(int v)
{
    if (v < 0){
        v += 12;
        m_yearBox->setValue(atol(m_yearBox->text().toLatin1()) - 1);
...
void PickerPopup::fill()
{
    ...
    if (month > 12)
        month -= 12;
    QDate d(atol(m_yearBox->text().toLatin1()), month, 1);
