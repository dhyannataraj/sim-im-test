...
string ICQClient::createRTF(const char *text, unsigned long foreColor, const char *encoding)
{
	QTextCodec *codec = getCodec(encoding);
	encoding = codec->name();
    ...
    for (;;){
        ...
        case WIDECHAR:
            ...
            bConvert = false;
            if (codec){
				QString utf  = QString::fromUtf8(t.c_str());
				string plain;
				plain = codec->fromUnicode(utf);
				if ((plain.length() == 1) && (codec->toUnicode(plain.c_str()) == utf)){
                        for (p = plain.c_str(); *p; p++){
                            if ((*p & 0x80) || (*p == '\\') || (*p == '{') || (*p == '}')){
                                char b[5];
                                snprintf(b, sizeof(b), "\\\'%02x", *p & 0xFF);
                                res += b;
