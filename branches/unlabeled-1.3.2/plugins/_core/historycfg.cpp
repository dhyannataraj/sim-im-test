...
void HistoryConfig::addStyles(const char *dir, bool bCustom)
{
	...
	for (QStringList::Iterator it = files.begin(); it != files.end(); ++it){
		...
		int n = name.findRev(".");
		name = name.left(n);
		for (vector<StyleDef>::iterator its = m_styles.begin(); its != m_styles.end(); ++its){
			if (name == (*its).name)
				break;
		}
		if (its == m_styles.end()){
...
void HistoryConfig::fillPreview()
{
	...
	Message m3;
	m3.setId((unsigned)(-3));
	m3.setText("Colored message");
	m3.setTime(now - 120);
	m3.setFlags(MESSAGE_SECURE | MESSAGE_URGENT);
