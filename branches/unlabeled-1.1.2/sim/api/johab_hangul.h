...
static bool
johab_hangul_wctomb (unsigned char *r, unsigned short wc)
{
...
static QString johab_hangul_decompose (unsigned short wc)
{
    QString res;
    unsigned char buf[2];
    if (!johab_hangul_wctomb(buf,wc))
        return res;
    ...
    if (jamo1 != NONE && jamo2 != NONE && jamo3 != NONE) {
        ...
        if (jamo1 != FILL)
            res += QChar((unsigned short)(0x3130 + jamo1));
        if (jamo2 != FILL)
            res += QChar((unsigned short)(0x3130 + jamo2));
        if (jamo3 != FILL)
            res += QChar((unsigned short)(0x3130 + jamo3));
    }
    return res;
}
