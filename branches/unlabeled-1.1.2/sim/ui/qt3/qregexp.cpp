...
/*
  The class MyQRegExpEngine encapsulates a modified nondeterministic
  finite automaton (NFA).
*/
class MyQRegExpEngine : public QShared
{
public:
	...
	*/
    ...
    MyQRegExpEngine( bool caseSensitive ) { setup( caseSensitive ); }
    MyQRegExpEngine( const QString& rx, bool caseSensitive );
#ifndef QT_NO_REGEXP_OPTIM
    ~MyQRegExpEngine();
#endif
...
private:
    ...
    */
    struct Lookahead
    {
	MyQRegExpEngine *eng; // NFA representing the embedded regular expression
	bool neg; // negative lookahead?

	Lookahead( MyQRegExpEngine *eng0, bool neg0 )
	    : eng( eng0 ), neg( neg0 ) { }
...
#ifndef QT_NO_REGEXP_LOOKAHEAD
    int addLookahead( MyQRegExpEngine *eng, bool negative );
#endif
    ...
    */
	...
	MyQRegExpEngine *eng; // the automaton under construction
...
MyQRegExpEngine::MyQRegExpEngine( const QString& rx, bool caseSensitive )
#ifndef QT_NO_REGEXP_BACKREF
    : mmSleeping( 101 )
#endif
{
...
#ifndef QT_NO_REGEXP_OPTIM
MyQRegExpEngine::~MyQRegExpEngine()
{
}
#endif
...
*/
void MyQRegExpEngine::match( const QString& str, int pos, bool minimal,
...
int MyQRegExpEngine::createState( QChar ch )
{
    return setupState( ch.unicode() );
}

int MyQRegExpEngine::createState( const CharClass& cc )
{
#ifndef QT_NO_REGEXP_CCLASS
    int n = cl.size();
    cl.resize( n + 1 );
...
#ifndef QT_NO_REGEXP_BACKREF
int MyQRegExpEngine::createState( int bref )
{
    if ( bref > nbrefs ) {
	nbrefs = bref;
...
void MyQRegExpEngine::addCatTransitions( const QMemArray<int>& from,
				       const QMemArray<int>& to )
{
...
#ifndef QT_NO_REGEXP_CAPTURE
void MyQRegExpEngine::addPlusTransitions( const QMemArray<int>& from,
					const QMemArray<int>& to, int atom )
{
    for ( int i = 0; i < (int) from.size(); i++ ) {
	State *st = s[from[i]];
...
*/
int MyQRegExpEngine::anchorAlternation( int a, int b )
{
...
*/
int MyQRegExpEngine::anchorConcatenation( int a, int b )
{
...
/*
  ...
  its to State.
*/
void MyQRegExpEngine::addAnchors( int from, int to, int a )
{
    State *st = s[from];
    if ( st->anchors == 0 )
...
*/
void MyQRegExpEngine::heuristicallyChooseHeuristic()
{
    if ( minl == 0 ) {
...
#if defined(QT_DEBUG)
void MyQRegExpEngine::dump() const
{
    int i, j;
...
void MyQRegExpEngine::setup( bool caseSensitive )
{
...
int MyQRegExpEngine::setupState( int match )
{
...
*/
int MyQRegExpEngine::startatom( bool capture )
{
...
*/
int MyQRegExpEngine::addLookahead( MyQRegExpEngine *eng, bool negative )
{
    int n = ahead.size();
    if ( n == MaxLookaheads ) {
	error( RXERR_LIMIT );
...
*/
bool MyQRegExpEngine::isBetterCapture( const int *begin1, const int *end1,
...
*/
bool MyQRegExpEngine::testAnchor( int i, int a, const int *capBegin )
{
    int j;
...
bool MyQRegExpEngine::goodStringMatch()
{
...
bool MyQRegExpEngine::badCharMatch()
{
    int slideHead = 0;
    int slideNext = 0;
    int i;
...
#else
bool MyQRegExpEngine::bruteMatch()
{
    while ( mmPos <= mmLen ) {
	if ( matchHere() )
	    return TRUE;
...
*/
bool MyQRegExpEngine::matchHere()
{
    int ncur = 1, nnext = 0;
    int i = 0, j, k, m;
    bool stop = FALSE;
...
MyQRegExpEngine::CharClass::CharClass()
    : c( 0 ), n( FALSE )
{
#ifndef QT_NO_REGEXP_OPTIM
...
MyQRegExpEngine::CharClass& MyQRegExpEngine::CharClass::operator=(
	const CharClass& cc )
{
    c = cc.c;
    r = cc.r.copy();
...
void MyQRegExpEngine::CharClass::clear()
{
    c = 0;
    r.resize( 0 );
    n = FALSE;
...
void MyQRegExpEngine::CharClass::setNegative( bool negative )
{
    n = negative;
#ifndef QT_NO_REGEXP_OPTIM
    occ1.fill( 0, NumBadChars );
...
void MyQRegExpEngine::CharClass::addCategories( int cats )
{
    c |= cats;
#ifndef QT_NO_REGEXP_OPTIM
    occ1.fill( 0, NumBadChars );
...
void MyQRegExpEngine::CharClass::addRange( ushort from, ushort to )
{
    if ( from > to )
	qSwap( from, to );
    int m = r.size();
...
bool MyQRegExpEngine::CharClass::in( QChar ch ) const
{
#ifndef QT_NO_REGEXP_OPTIM
...
#if defined(QT_DEBUG)
void MyQRegExpEngine::CharClass::dump() const
{
    int i;
...
MyQRegExpEngine::Box::Box( MyQRegExpEngine *engine )
...
MyQRegExpEngine::Box& MyQRegExpEngine::Box::operator=( const Box& b )
{
    eng = b.eng;
    ls = b.ls;
    rs = b.rs;
...
void MyQRegExpEngine::Box::set( QChar ch )
{
    ls.resize( 1 );
...
void MyQRegExpEngine::Box::set( const CharClass& cc )
{
    ls.resize( 1 );
...
#ifndef QT_NO_REGEXP_BACKREF
void MyQRegExpEngine::Box::set( int bref )
{
    ls.resize( 1 );
...
void MyQRegExpEngine::Box::cat( const Box& b )
{
...
void MyQRegExpEngine::Box::orx( const Box& b )
{
    mergeInto( &ls, b.ls );
    mergeInto( &lanchors, b.lanchors );
    mergeInto( &rs, b.rs );
...
void MyQRegExpEngine::Box::plus( int atom )
{
#ifndef QT_NO_REGEXP_CAPTURE
...
void MyQRegExpEngine::Box::catAnchor( int a )
{
    if ( a != 0 ) {
	for ( int i = 0; i < (int) rs.size(); i++ ) {
...
#ifndef QT_NO_REGEXP_OPTIM
void MyQRegExpEngine::Box::setupHeuristics()
{
...
#if defined(QT_DEBUG)
void MyQRegExpEngine::Box::dump() const
{
    int i;
...
void MyQRegExpEngine::Box::addAnchorsToEngine( const Box& to ) const
{
...
int MyQRegExpEngine::getChar()
{
    return ( yyPos == yyLen ) ? EOS : yyIn[yyPos++].unicode();
}

int MyQRegExpEngine::getEscape()
{
#ifndef QT_NO_REGEXP_ESCAPE
...
#ifndef QT_NO_REGEXP_INTERVAL
int MyQRegExpEngine::getRep( int def )
{
    if ( yyCh >= '0' && yyCh <= '9' ) {
	int rep = 0;
	do {
...
#ifndef QT_NO_REGEXP_LOOKAHEAD
void MyQRegExpEngine::skipChars( int n )
{
    if ( n > 0 ) {
	yyPos += n - 1;
	yyCh = getChar();
...
void MyQRegExpEngine::error( const char *msg )
{
    if ( yyError.isEmpty() )
	yyError = QString::fromLatin1( msg );
}

void MyQRegExpEngine::startTokenizer( const QChar *rx, int len )
{
    yyIn = rx;
    yyPos0 = 0;
    yyPos = 0;
...
int MyQRegExpEngine::parse( const QChar *pattern, int len )
{
    valid = TRUE;
...
void MyQRegExpEngine::parseatom( Box *box )
{
#ifndef QT_NO_REGEXP_LOOKAHEAD
    MyQRegExpEngine *eng = 0;
    bool neg;
    int len;
#endif
...
void MyQRegExpEngine::parseFactor( Box *box )
{
#ifndef QT_NO_REGEXP_CAPTURE
...
void MyQRegExpEngine::parseTerm( Box *box )
{
#ifndef QT_NO_REGEXP_OPTIM
...
void MyQRegExpEngine::parseExpression( Box *box )
{
    parseTerm( box );
    while ( yyTok == Tok_Bar ) {
#ifndef QT_NO_REGEXP_OPTIM
...
/*
  The struct QMyRegExpPrivate contains the private data of a regular
  expression other than the automaton. It makes it possible for many
  QRegExp objects to use the same MyQRegExpEngine object with different
  QMyRegExpPrivate objects.
*/
class MyQRegExpPrivate
{
public:
...
#endif
    QMemArray<int> captured; // what MyQRegExpEngine::search() returned last

    MyQRegExpPrivate() { captured.fill( -1, 2 ); }
};

static void regexpEngine( MyQRegExpEngine *&eng, const QString &pattern,
			  bool caseSensitive, bool deref )
{
    if ( !deref ) {
		eng = new MyQRegExpEngine( pattern, caseSensitive );
		return;
    }
...
{
    MyQRegExpEngine *otherEng = rx.eng;
    if ( otherEng != 0 )
	otherEng->ref();
    invalidateEngine();
    eng = otherEng;
