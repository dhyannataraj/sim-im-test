...
class Qt3::QSyntaxHighlighterPrivate
{
public:
...
class Qt3::QSyntaxHighlighterInternal : public QTextPreProcessor
{
public:
QSyntaxHighlighterInternal( QSyntaxHighlighter *h ) : highlighter( h ) {}
    void process( QTextDocument *doc, QTextParag *p, int, bool invalidate ) {
...
void QSyntaxHighlighter::setFormat( int start, int count, const QFont &font )
{
    if ( !para || count <= 0 )
        return;
    QTextFormat *f = 0;
    QColor c = textEdit()->viewport()->palette().active().text();
...
{
    QTextParag *s = edit->document()->firstParag();
    while ( s ) {
        s->invalidate( 0 );
        s->state = -1;
    ...
    edit->repaintContents( edit->visibleRect(), FALSE );
}
