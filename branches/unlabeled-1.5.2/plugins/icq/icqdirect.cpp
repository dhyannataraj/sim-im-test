...
void DirectClient::processPacket()
{
    ...
    case TCP_START:
        ...
        if (m_channel == PLUGIN_NULL){
            MessageId id;
            id.id_l = seq;
            m = m_client->parseMessage(type, m_client->screen(m_data).c_str(), msg_str, m_socket->readBuffer, 0, 0, id);
            if (m == NULL){
    ...
    case TCP_ACK:
        ...
        for (it = m_queue.begin(); it != m_queue.end(); ++it){
            ...
            MessageId id;
            id.id_l = seq;
            Message *m = m_client->parseMessage(type, m_client->screen(m_data).c_str(), msg_str, m_socket->readBuffer, 0, 0, id);
            switch (msg->type()){
#ifdef USE_OPENSSL
