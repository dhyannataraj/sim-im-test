...
AC_DEFUN(KDE_USE_QT,
[
AC_ARG_ENABLE(kde, [  --disable-kde           don't use kde],
        use_kde=$enableval, use_kde=yes)
if test "$use_kde" = "yes" ; then
  kde_default_bindirs="$KDEDIR/bin /usr/bin /usr/local/bin /opt/local/bin /usr/X11R6/bin /opt/kde/bin /opt/kde3/bin /opt/kde2/bin /usr/kde/bin /usr/local/kde/bin"
  KDE_FIND_PATH(kde-config, KDECONFIG, [$kde_default_bindirs])
...
AC_MSG_RESULT(${kde_qtver}.${kde_qtsubver})
else
...
if test $kde_qtver = 2; then
   kde_qt_dirs="$QTDIR /usr/lib/qt3 /usr/lib/qt2 /usr/lib/qt"
fi
])
...
AC_DEFINE(USE_KDE,1,[Use KDE])
else
kde_libraries="$qt_libraries"
AC_SUBST(kde_libraries)
fi
...
# these are for backward compatibility
   ...
   LIB_KDEPRINT='-lkdeprint'
   AC_SUBST(LIB_KDEPRINT)
else   LIB_KDECORE='-lkdecore -lXext $(LIB_QT)'
   AC_SUBST(LIB_KDECORE)
...
[AC_CACHE_CHECK([for obstacks], am_cv_func_obstack,
 ...
 if test $am_cv_func_obstack = yes; then
   AC_DEFINE(HAVE_OBSTACK, 1, [Have obstack])
 else
...
if test "$build_libtool_libs" = "yes" && test "$enable_dlopen" = "yes"; then
  dynamic_loading=yes
  AC_DEFINE_UNQUOTED(HAVE_DYNAMIC_LOADING, 1, [Have dunamic loading])
else
  dynamic_loading=no
fi
...
int Foo::Bar::i = 1;
  ...
  AC_MSG_RESULT(yes)
  AC_DEFINE(HAVE_NAMESPACES, 1, [Have namespaces])
], [
AC_MSG_RESULT(no)
])
AC_LANG_RESTORE
