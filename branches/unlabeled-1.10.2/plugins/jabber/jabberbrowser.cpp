    ...
    if (e->type() == static_cast<JabberPlugin*>(m_client->protocol()->plugin())->EventDiscoItem){
		...
		if (it){
            if (item->jid.empty()){
				...
				if (it->text(COL_ID_BROWSE).isEmpty()){
					QString err;
					if (!item->name.empty()){
			...
			if (it->firstChild() == NULL){
				...
				it->setOpen(it == m_list->firstChild());
			}
			QListViewItem *i;
			...
			i->setText(COL_ID_DISCO_ITEMS, m_client->discoItems(item->jid.c_str(), item->node.c_str()).c_str());
		    i->setText(COL_ID_DISCO_INFO, m_client->discoInfo(item->jid.c_str(), item->node.c_str()).c_str());
			if (item->node.empty())
				i->setText(COL_ID_BROWSE, m_client->browse(item->jid.c_str()).c_str());
            return e->param();
        }
		...
		if (it){
            if (item->jid.empty()){
				...
				if (it->text(COL_ID_DISCO_INFO).isEmpty()){
					QString err;
					if (!item->name.empty()){
						err = QString::fromUtf8(item->name.c_str());
					}else if (!item->node.empty()){
			...
			if (it->text(COL_JID) != QString::fromUtf8(item->jid.c_str())){
				...
				it->setText(COL_ID_DISCO_ITEMS, m_client->discoItems(item->jid.c_str(), item->node.c_str()).c_str());
				it->setText(COL_ID_DISCO_INFO, m_client->discoInfo(item->jid.c_str(), item->node.c_str()).c_str());
				it->setText(COL_ID_BROWSE, m_client->browse(item->jid.c_str()).c_str());
				}
			}
			if (it->text(COL_NAME) == it->text(COL_JID))
				it->setText(COL_NAME, QString::fromUtf8(item->name.c_str()));
