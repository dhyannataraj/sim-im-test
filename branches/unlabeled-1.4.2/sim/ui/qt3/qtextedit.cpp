...
QPopupMenu *QTextEdit::createPopupMenu( const QPoint& )
{
#ifndef QT_NO_POPUPMENU
    QPopupMenu *popup = new QPopupMenu( this, "qt_edit_menu" );
    if ( !isReadOnly() ) {
        d->id[ IdUndo ] = popup->insertItem( i18n( "&Undo" ) + ACCEL_KEY( Z ) );
        d->id[ IdRedo ] = popup->insertItem( i18n( "&Redo" ) + ACCEL_KEY( Y ) );
        popup->insertSeparator();
    }
#ifndef QT_NO_CLIPBOARD
    if ( !isReadOnly() )
        d->id[ IdCut ] = popup->insertItem( i18n( "Cu&t" ) + ACCEL_KEY( X ) );
    d->id[ IdCopy ] = popup->insertItem( i18n( "&Copy" ) + ACCEL_KEY( C ) );
    if ( !isReadOnly() )
        d->id[ IdPaste ] = popup->insertItem( i18n( "&Paste" ) + ACCEL_KEY( V ) );
#endif
    if ( !isReadOnly() ) {
        d->id[ IdClear ] = popup->insertItem( i18n( "Clear" ) );
        popup->insertSeparator();
    }
#if defined(Q_WS_X11)
