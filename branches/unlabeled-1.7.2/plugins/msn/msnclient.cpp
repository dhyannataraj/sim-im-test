...
void MSNClient::getLine(const char *line)
{
    ...
    unsigned id = getToken(l, ' ').toUInt();
    if (m_curPacket && (m_curPacket->id() != id)){
        delete m_curPacket;
        m_curPacket = NULL;
    }
    if (m_curPacket == NULL){
    ...
    if (m_curPacket == NULL){
        log(L_WARN, "Packet not found");
        return;
    }
    vector<string> args;
    while (l.length())
        args.push_back(string(getToken(l, ' ', false).utf8()));
    m_curPacket->answer(cmd.latin1(), args);
}
...
void MSNClient::authOk()
{
    ...
    packet->send();
    packet = new BlpPacket(this, getInvisible());
    packet->send();
    setStatus(m_logonStatus);
}
...
bool MSNClient::canSend(unsigned type, void *_data)
{
    ...
    case MessageAuthGranted:
        return (data->Flags & MSN_ACCEPT) == 0;
    case MessageAuthRefused:
        return (data->Flags & MSN_ACCEPT);
    }
    return false;
}
...
bool MSNClient::send(Message *msg, void *_data)
{
    ...
    case MessageAuthGranted:
        if ((data->Flags & MSN_ACCEPT) == 0){
            Contact *contact = getContacts()->contact(msg->contact());
            if (contact == NULL)
                return false;
            packet = new AddPacket(this, "AL", data->EMail, quote(contact->getName()).utf8());
        ...
        break;
    case MessageAuthRefused:
        if (data->Flags & MSN_ACCEPT){
            Contact *contact = getContacts()->contact(msg->contact());
            if (contact == NULL)
                return false;
            packet = new RemPacket(this, "AL", data->EMail);
            data->Flags &= ~MSN_ACCEPT;
            if ((msg->getFlags() & MESSAGE_NOHISTORY) == 0){
                Event e(EventSent, msg);
                e.process();
        ...
        case LR_CONTACTxCHANGED:
            ...
            if (data){
                ...
                if ((data->Flags & MSN_FORWARD) == 0){
                    packet = new AddPacket(this, "FL", data->EMail, quote(contact->getName()).utf8(), contact->getGroup());
