#/****************************************************************************
** $Id: qrichtext.cpp,v 1.14.2.1 2004-03-16 14:44:07 shutoff Exp $
**
...
void QTextFormat::removeRef()
{
    ...
    QTextParag::QTextParag( QTextDocument *d, QTextParag *pr, QTextParag *nx, bool updateIds )
            ...
            numSubParag( -1 ), tm( -1 ), bm( -1 ), lm( -1 ), rm( -1 ), flm( -1 ), tc( 0 ),
...
#define DO_FLOW( lineStart ) do{ if ( doc && doc->isPageBreakEnabled() ) { \
    ...
    static QString section(const QString &str, const QString &sep, int start, int end)
    {
        ...
        //find start
        int n = str.length(), sep_len = sep.length();
    ...
    bool endsWith(const QString &str, const QString &pat)
    {
        int n = str.length() - pat.length();
        if (n < 0)
            return false;
        return (str.mid(n) == pat);
    }
    ...
    QTextFormat QTextFormat::makeTextFormat( const QStyleSheetItem *style, const QMap<QString,QString>& attr) const
    {
        ...
        if ( attr.contains("style" ) ) {
            ...
            for ( int s = 0; s < a.contains(';')+1; s++ ) {
                QString style = section(a, ";", s, s );
                if ( style.startsWith("font-size:" ) && endsWith(style, "pt") ) {
                    format.logicalFontSize = 0;
                    int size = int( style.mid( 10, style.length() - 12 ).toDouble() );
...
#include "qcleanuphandler.h"
                                        ...
                                        QMap<QString, QString> &attr, bool& emptyTag)
    {
        ...
        eatSpace(doc, pos, TRUE);
        QString term = QString::fromLatin1("/>");
        QString s_TRUE = QString::fromLatin1("TRUE");
    ...
    void QTextTable::draw(QPainter* p, int x, int y, int cx, int cy, int cw, int ch, const QColorGroup& cg, bool selected )
    {
            ...
            if ( cx < 0 && cy < 0 ||
                ...
                if ( border ) {
                    ...
                    if ( is_printer( p ) || ( p && p->device() && p->device()->devType() == QInternal::Printer ) ) {
                        QPen oldPen = p->pen();
                        QRect r2 = QRect(r.left() + innerborder/2, r.top() + innerborder/2, r.right()-innerborder/2, r.bottom()-innerborder/2 );
        ...
        if ( border ) {
            QRect r ( x, y, width, height );
            if ( is_printer( p ) || ( p && p->device() && p->device()->devType() == QInternal::Printer ) ) {
                QRect r2 = QRect(r.left()+border/2, r.top()+border/2, r.right()-border/2, r.bottom()-border/2 );
                QPen oldPen = p->pen();
