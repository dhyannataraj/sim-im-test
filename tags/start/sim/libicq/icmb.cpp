    ...
    case ICQ_SNACxMSG_SERVERxMESSAGE:{
            ...
            if (uin == 0xA){
                ...
                if ((l[5].size() > strlen(SENDER_IP)) && !memcmp(l[5].c_str(), SENDER_IP, strlen(SENDER_IP))){
                    ...
                    m->Uin = contacts.findByEmail(m->Name, m->Email);
                    messageReceived(m);
                }else{
            ...
            case 0x0002:{
                    ...
                    case 1:{
                            ...
                            for (it = processQueue.begin(); it != processQueue.end(); ++it){
                                ...
                                if ((m->Uin() == uin) && (m->timestamp1 == timestamp1) && (m->timestamp2 == timestamp2)){
                ...
                if (m && (msgState == 0)){
                    ...
                    if (port && (ip || real_ip)){
                        ...
                        for (it = processQueue.begin(); it != processQueue.end(); ++it){
                            ...
                            sendThroughServer(m->Uin, 2, b, &id, false);

                            ICQUser *u = getUser(m->Uin);
                            if (u == NULL){
...
void ICQClient::acceptMessage(ICQMessage *m)
{
    ...
    if (m->Direct()){
        ...
        b.tlv(0x2711, msgBuf);
        sendThroughServer(m->Uin, 2, b, &id);
    }
    ICQEvent e(EVENT_ACKED, m->Uin());
    e.msg = m;
    process_event(&e);
    ...
    for (it = msgQueue.begin(); it != msgQueue.end();){
        ...
        case ICQ_MSGxMSG:{
                ...
                if (u && u->GetRTF && (u->uStatus != ICQ_STATUS_OFFLINE)){
                    ...
                    b.tlv(0x0F);
                    b.tlv(0x2711, msgBuf);
                    sendThroughServer(msg->Uin, 2, b, &id);
                }else{
        ...
        case ICQ_MSGxFILE:{
                ...
                b.tlv(0x2711, mb);
                sendThroughServer(file->Uin, 2, b, &id);
                msgQueue.remove(e);
        ...
        case ICQ_MSGxCHAT:{
                ...
                b.tlv(0x05, (unsigned short)Port());
                b.tlv(0x2711, mb);
                sendThroughServer(chat->Uin, 2, b, &id);
                msgQueue.remove(e);
                processQueue.push_back(e);
                it = msgQueue.begin();
                bToStart = true;
